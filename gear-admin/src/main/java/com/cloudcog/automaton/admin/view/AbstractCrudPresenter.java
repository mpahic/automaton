...
public abstract class AbstractCrudPresenter<T extends AbstractEntity, S extends CrudService<T>, V extends AbstractCrudView<T>>
	...
	private static final long serialVersionUID = 3149961973166615564L;

	private static final Logger log = LoggerFactory.getLogger(AbstractCrudPresenter.class);
	...
	public void updateClicked() {
		...
		try {
			...
			Notification.show("Somebody else might have updated the data. Please refresh and try again.",
					Type.ERROR_MESSAGE);
			log.debug("Optimistic locking error while saving entity of type " + editItem.getClass().getName(),
					e);
			return;
		} catch (UserFriendlyDataException e) {
			Notification.show(e.getMessage(), Type.ERROR_MESSAGE);
			log.debug("Unable to update entity of type " + editItem.getClass().getName(), e);
			return;
		} catch (Exception e) {
			...
			log.error("Unable to save entity of type " + editItem.getClass().getName(), e);
			return;
		}
	...
	public void deleteClicked() {
		try {
			...
			Notification.show(e.getMessage(), Type.ERROR_MESSAGE);
			log.warn("Unable to delete entity of type " + editItem.getClass().getName(), e);
			return;
		} catch (DataIntegrityViolationException e) {
			Notification.show("The given entity cannot be deleted as there are references to it in the database",
					Type.ERROR_MESSAGE);
			log.error("Unable to delete entity of type " + editItem.getClass().getName(), e);
			return;
		}
		dataProvider.refreshAll();
		revertToInitialState();
